-- ============================================================
-- MeToca - Fix redistribution window and balance by pending load
-- ============================================================
...
begin
  ...
  begin
    v_month_start := to_date(trim(p_month) || '-01', 'YYYY-MM-DD');
  exception
    when others then
  ...
  v_generation_start := greatest(v_month_start, current_date);
  ...
  loop
    ...
    -- Choose the least loaded member (future pending tasks in this household/month).
    select ranked.user_id
    into v_selected_member_id
    from (
      select
        tm.user_id,
        count(tk.id) as pending_load,
        tm.ordinality
      from unnest(v_target_member_ids) with ordinality as tm(user_id, ordinality)
      left join public.task_executions te
        on te.assigned_to = tm.user_id
       and te.status = 'pending'
       and te.scheduled_date >= v_generation_start
       and te.scheduled_date <= v_month_end
      left join public.tasks tk
        on tk.id = te.task_id
       and tk.household_id = p_household_id
      group by tm.user_id, tm.ordinality
      order by pending_load asc, tm.ordinality asc
      limit 1
    ) as ranked;

    if v_selected_member_id is null then
      v_selected_member_id := v_target_member_ids[1];
    end if;

    insert into public.task_assignments (task_id, household_id, user_id, month)
    values (v_task.id, p_household_id, v_selected_member_id, p_month)
    ...
    set
      assigned_to = v_selected_member_id,
    ...
    where te.task_id = v_task.id
      ...
      and te.status = 'pending'
      and (
        te.assigned_to is distinct from v_selected_member_id
    ...
    loop
      if v_schedule.frequency = 'daily' then
        ...
        while v_date <= v_month_end loop
          ...
          select v_assignment_id, v_task.id, v_selected_member_id, v_date, v_schedule.notification_time, 'pending'
          where not exists (
            select 1
        ...
        if v_date >= v_generation_start and v_date <= v_month_end then
          insert into public.task_executions (task_assignment_id, task_id, assigned_to, scheduled_date, scheduled_time, status)
          select v_assignment_id, v_task.id, v_selected_member_id, v_date, v_schedule.notification_time, 'pending'
          where not exists (
            select 1
  ...
  return jsonb_build_object(
    ...
    'month', trim(p_month),
    'generation_start', v_generation_start,
