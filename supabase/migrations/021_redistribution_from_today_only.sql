...
begin
  ...
  select array_agg(hm.user_id order by coalesce(hm.joined_at, hm.created_at), hm.user_id), count(*)
  ...
  loop
    ...
    values (v_task.id, p_household_id, v_member_ids[v_member_index], p_month)
    ...
    set
      assigned_to = v_member_ids[v_member_index],
    ...
    where te.task_id = v_task.id
      ...
      and (
        te.assigned_to is distinct from v_member_ids[v_member_index]
    ...
    loop
      if v_schedule.frequency = 'daily' then
        ...
        while v_date <= v_month_end loop
          ...
          select v_assignment_id, v_task.id, v_member_ids[v_member_index], v_date, v_schedule.notification_time, 'pending'
          where not exists (
            select 1
